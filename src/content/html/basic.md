...
- OGP内に記述するURLや画像パスは、外部のSNSサーバーが読み取りにくるため、すべて**絶対パス**（`https://...`）で記述してください。
...
* **URLの指定：** `item` プロパティに指定するURLは、必ずプロトコル（`https://`）から始まる**絶対パス**で記述してください。
* **位置の連番：** `position` プロパティは、トップページ（ホーム）を `1` とし、下層にいくにつれて `2`, `3` と1ずつ増える連番で記述します。
* **画面表示との一致：** 構造化データ内の `name`（ページ名）や階層順は、**実際の画面上に表示されているパンくずリストの内容と完全に一致**させてください。
...
HTML5の仕様（W3C/WHATWG規格）上、一部の要素（`<li>`, `<td>`, `<p>` など）は、次に別の特定のタグが続く場合に閉じタグを省略することが認められています。しかし、当社の開発においては「**すべてのペアタグの閉じタグを絶対に省略しない**」を共通ルールとします。
...
これらは歴史的な経緯（HTML4/XHTML/HTML5の過渡期）により複数の書き方が存在しますが、コードの一貫性を保つため、**末尾のスラッシュを「入れない」表記に統一します**。
