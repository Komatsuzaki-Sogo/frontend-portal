...
* **`innerHTML` および `insertAdjacentHTML` の原則禁止（MUST NOT）：**
...
* **`textContent` または `innerText` の標準化（MUST）：**
